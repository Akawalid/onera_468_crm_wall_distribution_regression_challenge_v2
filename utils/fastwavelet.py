...
WAVELET = 'haar'
MAX_LEVELS = 5
GRID_SIZES = {'wing': 128, 'pylon': 128, 'fuselage': 64, 'nacelle': 64}
...
def build_grid_mapping(uv, n_grid):
    ...
    return cell_idx, counts, filled, fill_for_empty, ui, vi
...
def multiscale_maps(grid, max_levels):
    n = grid.shape[0]
    coeffs = pywt.wavedec2(grid, WAVELET)
    detail_levels = coeffs[1:]
    detail_levels = detail_levels[::-1]
    maps = []
    for lvl in range(min(max_levels, len(detail_levels))):
        cH, cV, cD = detail_levels[lvl]
        f = n // cH.shape[0]
        for arr in (cH, cV, cD):
            maps.append(np.kron(arr, np.ones((f, f)))[:n, :n])
    cA = coeffs[0]
    f = n // cA.shape[0]
    maps.append(np.kron(cA, np.ones((f, f)))[:n, :n])
    while len(maps) < 3 * max_levels + 1:
        maps.append(np.zeros((n, n)))
    return np.stack(maps)


def build_point_features():
    X_ref = X_train[:nwallp]
    coords = X_ref[:, :3].astype(np.float64)
    normals = X_ref[:, 3:6].astype(np.float64)
    ...
    n_wavefeat = 3 * MAX_LEVELS + 1
    wf_mean = np.zeros((nwallp, n_wavefeat), dtype=np.float32)
    wf_std = np.zeros((nwallp, n_wavefeat), dtype=np.float32)
    uv_feat = np.zeros((nwallp, 2), dtype=np.float32)

    for cname, cmask in comp_masks.items():
        if cname not in KL_WEIGHTS:
            continue
        n_grid = GRID_SIZES[cname]
        for side_mask in (cmask & upper_ref, cmask & (~upper_ref)):
            if side_mask.sum() == 0:
                continue
            uv = pca_project_2d(coords[side_mask])
            cell_idx, counts, filled, fill_for_empty, ui, vi = build_grid_mapping(uv, n_grid)
            uv_feat[side_mask, 0] = ui / n_grid
            uv_feat[side_mask, 1] = vi / n_grid

            g_mean = encode_grid(mean_field[side_mask], cell_idx, counts, filled, fill_for_empty, n_grid)
            g_std = encode_grid(std_field[side_mask], cell_idx, counts, filled, fill_for_empty, n_grid)
            maps_mean = multiscale_maps(g_mean, MAX_LEVELS)
            maps_std = multiscale_maps(g_std, MAX_LEVELS)
            wf_mean[side_mask] = maps_mean.reshape(n_wavefeat, -1)[:, cell_idx].T
            wf_std[side_mask] = maps_std.reshape(n_wavefeat, -1)[:, cell_idx].T

    static = np.column_stack([
        coords, normals, uv_feat,
        mean_field, std_field,
        wf_mean, wf_std,
    ]).astype(np.float32)
    print(f'static feature matrix: {static.shape}', flush=True)
...
def build_train_set(static, mean_field):
    ...
    for s in range(n_train):
        ...
        Xs.append(np.hstack([static[pts], f_conds]))
...
def predict_phase(model, static, conds, n_sims, mean_field):
    ...
    for s in range(n_sims):
        ...
        y_pred[s * nwallp:(s + 1) * nwallp] = model.predict(F) + mean_field
    return y_pred


def main():
    static, mean_field = build_point_features()
    ...
    n_wavefeat = 3 * MAX_LEVELS + 1
    feat_names = ['x', 'y', 'z', 'nx', 'ny', 'nz', 'u', 'v', 'mean_rho', 'std_rho']
    for prefix in ('wmean', 'wstd'):
        for lvl in range(MAX_LEVELS):
            feat_names += [f'{prefix}_H{lvl}', f'{prefix}_V{lvl}', f'{prefix}_D{lvl}']
        feat_names += [f'{prefix}_A']
    feat_names += ['Minf', 'AoA', 'Pi']
    imp = model.feature_importances_
    order = np.argsort(imp)[::-1]
    print('\nFeature importances (top 20):')
    for j in order[:20]:
        print(f'  {feat_names[j]:<12} {imp[j]}')
    ...
    np.save('y_pred1_wavfeat.npy', y_pred1)
    np.save('y_pred2_wavfeat.npy', y_pred2)
    ...
    print_phase_summary('Phase 1 (LightGBM + multiscale wavelet features)', res1)
    print_phase_summary('Phase 2 (LightGBM + multiscale wavelet features)', res2)
