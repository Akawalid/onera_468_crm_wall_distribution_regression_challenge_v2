"""v2 of train_mlp_with_kl.py: the training loss (klw_loss) now decouples P's histogram range
from Q's reference width, instead of tying both to one shared sigma_s.

v1 used a single global sigma_s both to normalize residuals into the soft-histogram (P's range)
and, implicitly, to set the reference Gaussian Q's absolute width (Q = N(0, SIGMA_REF*sigma_s) in
that same normalized space) -- so P's range was global (not tracking each simulation's own
scale, unlike scoring_program/scoring.py's per-simulation lim=5*std), while Q's width, at least,
correctly stayed global (matching scoring.py's global sigma_ref). Naively swapping in a
per-simulation sigma_s (as an earlier, now-removed version of this file did) would have fixed
P's range but broken Q's width, letting noisier/high-AoA simulations get an automatically more
lenient target just for being naturally noisier -- exactly what this metric is designed to not
allow (see bundle/pages/evaluation.md).

Here P and Q use two different, independent scales: P is normalized by each simulation's own std
(sigma_train, computed once from y_train), Q's absolute width stays pinned to the single global
sigma_ref (0.01 * mean_global) regardless of that simulation's own scale, re-expressed per sample
in whatever normalized units P happens to be using.
"""
...
def residual_kl_normal(y_true, y_pred, sigma_ref_frac=0.1, n_bins=200):
    ...
    # Histogram range tracks this simulation's own residual/target scale (matches
    # scoring_program/scoring.py's _residual_kl), not the global sigma_s.
...
def main():
    ...
    SIGMA_SCALE = 10.0 * 0.01 * mean_global          # kept for residual_kl_*'s bias/spread diagnostics only
    sigma_ref   = 0.01 * mean_global                 # Q's fixed absolute width, matches scoring.py's global sigma_ref
    print(f'global train mean = {mean_global:.4f}')
    print(f'sigma_ref (fixed Q width) = {sigma_ref:.5f}')
    ...
    def klw_loss(y_pred, y_true, sigma_y):
        """sigma_y: per-simulation std (P's range) -- Q's width stays pinned to the global
        sigma_ref regardless of sigma_y, re-expressed per sample in sigma_y-normalized units."""
        ...
        q_width = sigma_ref / sigma_y.unsqueeze(1)
        q = torch.exp(-0.5 * (centers.view(1, -1) / q_width) ** 2)
    ...
    # Per-simulation std (P's range), NOT a global constant -- see module docstring.
    sigma_train = Y_train_t.std(dim=1) + 1e-6
    ...
    mean_field = Y_train_t[tr_idx_t].mean(dim=0).cpu().numpy()
    ...
    print('Training MLP (loss = sum of per simulation weighted KL, per-sim range / global reference)...')
    ...
    torch.save({'state_dict': model.state_dict(),
                ...
                'hidden': HIDDEN, 'dropout': DROPOUT, 'nwallp': nwallp,
                'sigma_scale': SIGMA_SCALE, 'sigma_ref': sigma_ref, 'mean_global': mean_global,
    ...
    for label, res in [('Phase 1 (global MLP, per-sim range / global ref)', res1), ('Phase 2 (global MLP, per-sim range / global ref)', res2)]:
